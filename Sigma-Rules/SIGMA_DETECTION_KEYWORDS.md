...
> This document covers **only the `detection:` section** of Sigma rules —
...
Block names are arbitrary — only `condition` and `timeframe` are reserved.
...
Used with `keywords` block name — matches anywhere in the log:
```yaml
keywords:
  - 'mimikatz'
  - 'sekurlsa'
...
*1,167 rules — maps to MiniEDR `processCreate`*
...
- `CommandLine|contains` — 914 rules (substring match)
- `CommandLine|contains|all` — 409 rules (all substrings must be present)
- `CommandLine|contains|windash` — 92 rules (treats `-` and `/` as equivalent)
- `CommandLine|endswith` — 47 rules
- `CommandLine|re` — 36 rules (regex)

---

### 3.2 `registry_set` / `registry_event`
*204 + 32 rules — maps to MiniEDR `regValueSet`*
...
*160 + 33 + 9 rules — maps to MiniEDR `powershellScript`*
...
### 3.4 `dns_query`
*22 rules — maps to MiniEDR `dnsQuery`*
...
*51 rules — maps to MiniEDR `networkConnect`*
...
*11 rules — maps to MiniEDR `remoteThread`*
...
### 3.7 `image_load`
*98 rules — maps to MiniEDR `imageLoad`*
...
### 3.8 `wmi_event`
*3 rules — maps to MiniEDR `wmiActivity`*
...
*165 + 12 + 7 rules — **NOT supported in MiniEDR v45** (no file write/create ETW)*
...
### 3.10 `process_access`
*23 rules — **NOT supported in MiniEDR v45** (no OpenProcess ETW, only LSASS)*
...
### 3.11 `pipe_created`
*17 rules — **NOT supported in MiniEDR v45** (no named pipe ETW)*
...
### 3.12 `driver_load`
*10 rules — **NOT supported in MiniEDR v45** (no driver load ETW)*
...
| `not` | 747 | Negation — block must NOT match |
...
- `1 of selection_*` — matches any block starting with `selection_`
- `all of filter_*` — matches all blocks starting with `filter_`
- `1 of them` — matches any block (equivalent to `1 of *`)
...
### 8.2 Unsupported Fields (Sigma → MiniEDR gap)

| Sigma Field | Category | Why Not Supported |
|---|---|---|
| `OriginalFileName` | process_creation, image_load | Requires PE metadata parsing from ETW — not available in kernel process provider |
| `Hashes` | process_creation, image_load | Requires hash computation — not done in real-time ETW stream |
| `IntegrityLevel` | process_creation | Not exposed by kernel process ETW provider (needs token query) |
| `Description`, `Product`, `Company` | process_creation | PE version info not in ETW — requires file read at process start |
| `ParentCommandLine` | process_creation | Parent cmdline not in child's ETW event — requires separate lookup |
| `Signed`, `SignatureStatus` | image_load | Signature validation not in ETW — requires WinVerifyTrust call |
| `Initiated` | network_connection | MiniEDR captures all connections — no inbound/outbound flag |
...
| `cidr` modifier | network_connection | No CIDR range matching — only substring |
| `re` modifier | all | No regex engine — substring only |
...
| `registry_delete` | Only RegSetValue (opcode 10) captured — not RegDeleteValue |
| `create_stream_hash` | No Alternate Data Stream monitoring |
| `process_tampering` | No process hollowing/doppelgänging detection |
...
|---|---|---|---|
| ✅ Converted | ~1,715 | 72% | Successfully converted by `sigma_to_myedr.py` |
| ⚠️ Partial loss | ~400 | 17% | Converted but missing fields like `OriginalFileName`, `IntegrityLevel` |
| ❌ Unsupported | ~668 | 28% | Category not supported in MiniEDR v45 |
...
| `informational` | 10 | Low-signal rules — skipped by default |
...
```
SIGMA FIELD          →  MINIEDR FIELD         EVENT TYPE
─────────────────────────────────────────────────────────
Image                →  child_image           processCreate
ParentImage          →  parent_image          processCreate
CommandLine          →  cmdline_contains      processCreate
CurrentDirectory     →  child_path            processCreate
TargetObject         →  registry              regValueSet
Details              →  registry              regValueSet
ScriptBlockText      →  script_contains       powershellScript
QueryName            →  domain                dnsQuery
Image (dns)          →  dns_process           dnsQuery
DestinationIp        →  dest_ip               networkConnect
DestinationHostname  →  dest_ip               networkConnect
DestinationPort      →  dest_port             networkConnect
SourceImage          →  parent_image          remoteThread
TargetImage          →  child_image           remoteThread
ImageLoaded          →  image_file            imageLoad
Image (imgload)      →  image_process         imageLoad
Destination          →  query_contains        wmiActivity

SIGMA MODIFIER       →  MINIEDR EQUIVALENT
─────────────────────────────────────────────────────────
|contains            →  substring match (default behaviour)
|endswith            →  substring match (converter strips path)
|startswith          →  substring match
|contains|all        →  multiple values joined with AND
|contains|windash    →  converted as plain contains
|re                  →  ❌ not supported (skipped)
|cidr                →  ❌ not supported (skipped)
|base64offset        →  ❌ not supported (skipped)

SIGMA CONDITION      →  MINIEDR EQUIVALENT
─────────────────────────────────────────────────────────
selection            →  all fields in one BRule
and                  →  multiple fields in same BRule
or                   →  separate | in field value
not filter           →  ❌ filters dropped (not converted)
1 of selection_*     →  separate BRule per selection block
```
